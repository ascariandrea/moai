...
public class BlurBitmapHelper {
    ...
    public static Bitmap blurred(Bitmap source) {
        int i = 0;
        for (i = 0; i < scales.length && source.getWidth() > MIN_SIZE && source.getHeight() > MIN_SIZE; i++)
            source =
                    Bitmap.createScaledBitmap(source, source.getWidth() / scales[i], source.getHeight()
                            / scales[i], true);
        for (i--; i >= 0; i--)
            source =
                    Bitmap.createScaledBitmap(source, source.getWidth() * scales[i], source.getHeight()
